 ...
 */
function procesarHtmlPersonalizado(htmlString) {
...
async function cargarDetalleCurso() {
    ...
    const avatarElem = document.getElementById('courseInstructorAvatar');
    ...
    const sidebarImageElem = document.getElementById('courseSidebarImage');
    ...
    try {
        ...
        if (titleElem)
            titleElem.textContent = curso.titulo;
        if (instructorElem) {
            if (curso.instructor && curso.instructor.trim() !== '' && curso.instructor !== 'Varios' && !curso.instructor.includes('Opcional')) {
                ...
                if (parentMeta)
                    parentMeta.style.display = 'inline-flex';
            }
            else {
                const parentMeta = instructorElem.closest('.course-detail__meta-item');
                if (parentMeta)
        ...
        if (descElem)
            descElem.textContent = curso.descripcion || 'Descripción no disponible';
        if (priceElem)
            priceElem.textContent = curso.precio.toFixed(2);
        // Inyectar Descuento y Precio Anterior
        if (curso.precio_original && curso.precio_original > curso.precio) {
            if (discountContainer)
                discountContainer.style.display = 'flex';
            if (coursePriceOriginal)
                coursePriceOriginal.textContent = `$${curso.precio_original.toFixed(2)}`;
        ...
        const instructorSectionAvatar = document.getElementById('instructorSectionAvatar');
        ...
        if (instructorSection && hasInstructorInfo) {
            ...
            if (instructorSectionName)
                instructorSectionName.textContent = curso.instructor || '';
            if (instructorSectionBio)
                instructorSectionBio.textContent = curso.instructor_bio || '';
            if (instructorSectionAvatar && curso.instructor_avatar && curso.instructor_avatar.trim() !== '') {
                ...
                if (instructorAvatarWrapper)
                    instructorAvatarWrapper.style.display = 'block';
            }
            else {
                if (instructorAvatarWrapper)
                    instructorAvatarWrapper.style.display = 'none';
            }
        }
        ...
        mostrarError(`Ocurrió un error al cargar la información del curso: ${error.message}`);
    }
}
/**
