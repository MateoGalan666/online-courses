...
let leccionesActivas = [];
let deberesActivos = [];
let usuarioRol = '';
let editingLessonId = null;
let editingAssignmentId = null;
let currentLeccionId = null;
// ──────────────── INIT ────────────────
document.addEventListener('DOMContentLoaded', () => {
    ...
    cargarMisCursos().then(() => {
        ...
        if (cursoId) {
            const item = document.querySelector(`.student-course-item[data-curso-id="${cursoId}"]`);
            if (item)
                item.click();
        }
    });
    ...
    const htmlArea = document.getElementById('lessonContentField');
...
function verificarAcceso() {
    ...
    try {
        const user = JSON.parse(userJson);
        if (!['estudiante', 'profesor', 'admin'].includes(user.rol))
...
// ──────────────── LECCIONES ────────────────
async function cargarLecciones(cursoId, activarLessonId) {
    const token = localStorage.getItem(TOKEN_KEY);
    const lessonsList = document.getElementById('lessonsList');
    if (lessonsList)
        lessonsList.innerHTML = '<div class="loading-spinner" style="margin:2rem auto;"></div>';
    try {
        ...
        if (lessonsList)
            lessonsList.innerHTML = `<div style="color:var(--danger);text-align:center;">Error al cargar lecciones.</div>`;
    }
}
function renderBrowserTabs(lecciones, activarLessonId) {
...
function mostrarLeccion(leccion) {
    ...
    if (titleEl)
        titleEl.textContent = leccion.titulo;
    if (descEl)
        descEl.textContent = leccion.descripcion || '';
...
function renderTemario(lecciones) {
    ...
    Object.keys(porDia).map(Number).sort((a, b) => a - b).forEach((dia, di) => {
        ...
        header.addEventListener('click', () => {
            ...
            if (!open)
                dayDiv.classList.add('day-group--open');
        });
        ...
        lessons.forEach(leccion => {
            ...
            li.addEventListener('click', () => {
                ...
                document.querySelectorAll('.browser-tab').forEach(t => {
                    const tab = t;
                    if (tab.dataset.lessonId === leccion.id.toString())
                        tab.classList.add('browser-tab--active');
                    else
...
// ──────────────── MODAL CLASE ────────────────
function abrirModalClase(leccion) {
    ...
    if (leccion) {
        ...
        document.getElementById('lessonIdField').value = leccion.id.toString();
        document.getElementById('lessonTitleField').value = leccion.titulo;
        document.getElementById('lessonDayField').value = (leccion.dia || 1).toString();
        document.getElementById('lessonVideoField').value = leccion.video_url || '';
        const html = leccion.contenido_html || '';
        document.getElementById('lessonContentField').value = html;
        ...
        if (btnSave)
            btnSave.textContent = 'Crear Clase';
        document.getElementById('lessonIdField').value = '';
        document.getElementById('lessonTitleField').value = '';
        document.getElementById('lessonDayField').value = '1';
        document.getElementById('lessonVideoField').value = '';
        document.getElementById('lessonContentField').value = '';
        const visual = document.getElementById('editorVisual');
        if (visual)
            visual.innerHTML = '';
    }
...
async function guardarClase(e) {
    ...
    if (errorBox)
        errorBox.style.display = 'none';
    const titulo = document.getElementById('lessonTitleField').value.trim();
    const dia = parseInt(document.getElementById('lessonDayField').value) || 1;
    const videoUrl = document.getElementById('lessonVideoField').value.trim();
    const token = localStorage.getItem(TOKEN_KEY);
    ...
    let contenido = '';
    if (tabHtmlActive) {
        contenido = document.getElementById('lessonContentField').value;
    }
    else {
        contenido = document.getElementById('editorVisual').innerHTML || '';
        // Limpiar si solo hay placeholder vacío
        if (contenido === '<br>' || contenido.trim() === '')
            contenido = '';
    }
    if (!titulo) {
    ...
    try {
        ...
        cerrarModalClase();
        if (cursoActivoId)
            await cargarLecciones(cursoActivoId, data.id);
    }
    catch (err) {
...
function setEditorMode(mode) {
    ...
    const visual = document.getElementById('editorVisual');
    const html = document.getElementById('lessonContentField');
    if (mode === 'visual') {
        btnV.classList.add('active');
        btnH.classList.remove('active');
        toolV.style.display = 'flex';
        ...
        toolH.style.display = 'flex';
        toolV.style.display = 'none';
...
window.execFormatCmd = function (cmd) {
    document.execCommand(cmd, false);
    document.getElementById('editorVisual')?.focus();
};
window.insertVisual = function (tag) {
...
window.insertHtmlTag = function (tag) {
    const ta = document.getElementById('lessonContentField');
    if (!ta)
        return;
...
// ──────────────── DEBERES ────────────────
async function cargarDeberes(cursoId) {
...
window.editarDeber = function (id) { abrirModalDeber(deberesActivos.find(d => d.id === id)); };
window.eliminarDeber = async function (id) {
    if (!confirm('¿Eliminar este deber?'))
        return;
    const token = localStorage.getItem(TOKEN_KEY);
    try {
        ...
        if (cursoActivoId)
            await cargarDeberes(cursoActivoId);
    }
    catch (err) {
        alert(err.message);
    }
};
function abrirModalDeber(deber) {
    ...
    if (deber) {
        if (title)
            title.textContent = 'Editar Deber';
        if (btnSave)
            btnSave.textContent = 'Guardar Cambios';
        document.getElementById('assignmentIdField').value = deber.id.toString();
        document.getElementById('assignmentTitleField').value = deber.titulo;
        document.getElementById('assignmentDescField').value = deber.descripcion;
        document.getElementById('assignmentDeadlineField').value = deber.fecha_limite || '';
    }
    else {
        if (title)
            title.textContent = 'Nuevo Deber';
        if (btnSave)
            btnSave.textContent = 'Crear Deber';
        document.getElementById('assignmentIdField').value = '';
        document.getElementById('assignmentTitleField').value = '';
        document.getElementById('assignmentDescField').value = '';
        document.getElementById('assignmentDeadlineField').value = '';
    }
...
async function guardarDeber(e) {
    ...
    if (errBox)
        errBox.style.display = 'none';
    const titulo = document.getElementById('assignmentTitleField').value.trim();
    const descripcion = document.getElementById('assignmentDescField').value.trim();
    const deadline = document.getElementById('assignmentDeadlineField').value;
    const token = localStorage.getItem(TOKEN_KEY);
    if (!titulo || !descripcion) {
        showError(errBox, 'El título y la descripción son obligatorios.');
        return;
    }
...
window.abrirModalEntrega = function (assignmentId) {
    const errBox = document.getElementById('deliveryError');
    const idField = document.getElementById('deliveryAssignmentId');
    const content = document.getElementById('deliveryContentField');
    if (errBox)
        errBox.style.display = 'none';
    if (idField)
        idField.value = assignmentId.toString();
    const deber = deberesActivos.find(d => d.id === assignmentId);
    if (content)
        content.value = deber?.contenido_respuesta || '';
...
async function enviarEntrega(e) {
    e.preventDefault();
    const errBox = document.getElementById('deliveryError');
    if (errBox)
        errBox.style.display = 'none';
    const assignmentId = document.getElementById('deliveryAssignmentId').value;
    const contenido = document.getElementById('deliveryContentField').value.trim();
    const fileInput = document.getElementById('deliveryFileField');
    const token = localStorage.getItem(TOKEN_KEY);
...
// ──────────────── QUIZ ESTUDIANTE ────────────────
async function cargarQuizEstudiante(leccionId) {
...
window.verificarRespuesta = function (btn, correcta) {
    const container = btn.parentElement;
    const radios = container.querySelectorAll('input[type="radio"]');
    let seleccionada = -1;
    radios.forEach((r) => { if (r.checked)
        seleccionada = parseInt(r.value); });
    const feedback = container.querySelector('.respuesta-feedback');
    if (!feedback)
        return;
...
// ──────────────── QUIZ PROFESOR ────────────────
async function cargarPreguntasProfesor(leccionId) {
    ...
    try {
        ...
        <div>
          ...
          <div class="question-item__meta">${q.opciones.map((o, i) => `${i === q.respuesta_correcta ? '✅ ' : ''}${escapeHTML(o)}`).join(' &nbsp;|&nbsp; ')}</div>
        </div>
        ...
        list.innerHTML = `<p style="color:var(--danger);font-size:0.85rem;">Error al cargar preguntas.</p>`;
    }
}
window.eliminarPregunta = async function (id) {
    ...
    try {
        ...
        if (currentLeccionId)
            cargarPreguntasProfesor(currentLeccionId);
    }
    catch (err) {
        alert(err.message);
    }
...
function abrirModalPregunta() {
    ...
    document.querySelector('input[name="correctaRadio"][value="0"]').checked = true;
...
async function guardarPregunta(e) {
    ...
    if (errBox)
        errBox.style.display = 'none';
    const pregunta = document.getElementById('questionTextField').value.trim();
    const opciones = ['opcion0', 'opcion1', 'opcion2', 'opcion3']
        .map(id => document.getElementById(id).value.trim())
        .filter(o => o !== '');
    const correctaEl = document.querySelector('input[name="correctaRadio"]:checked');
    const respuesta_correcta = parseInt(correctaEl?.value ?? '0');
    if (!pregunta) {
        showError(errBox, 'La pregunta es obligatoria.');
        return;
    }
    if (opciones.length < 2) {
        showError(errBox, 'Debes ingresar al menos 2 opciones.');
        return;
    }
    if (respuesta_correcta >= opciones.length) {
        showError(errBox, 'La opción correcta seleccionada no existe.');
        return;
    }
    const token = localStorage.getItem(TOKEN_KEY);
    try {
