...
async function iniciarCheckout() {
    ...
    if (inputNombre) {
        ...
        if (previewName)
            previewName.textContent = user.nombre.toUpperCase();
    }
    if (inputEmail) {
 ...
 */
async function cargarResumenPedido(cursoId) {
    ...
    const imgElem = document.getElementById('summaryCourseImg');
    try {
        ...
        if (subtotalElem)
            subtotalElem.textContent = `$${curso.precio.toFixed(2)}`;
        if (totalElem)
            totalElem.textContent = `$${curso.precio.toFixed(2)}`;
        if (imgElem) {
...
function configurarTarjeteroEnVivo() {
    ...
    const inputExpiracion = document.getElementById('expiracionTarjeta');
    const previewName = document.getElementById('previewName');
 ...
 */
async function procesarPagoSimulado(e, cursoId) {
    e.preventDefault();
    const nombreCliente = document.getElementById('nombreCliente').value;
    const emailCliente = document.getElementById('emailCliente').value;
    const numeroTarjeta = document.getElementById('numeroTarjeta').value;
    const expiracionTarjeta = document.getElementById('expiracionTarjeta').value;
    ...
    if (btnSubmitText)
        btnSubmitText.textContent = 'Procesando Pago con Pasarela de Stripe...';
    if (btnSubmitSpinner)
    ...
    try {
        ...
        if (btnSubmitText)
            btnSubmitText.textContent = 'Proceder con el Pago Simulado';
        if (btnSubmitSpinner)
