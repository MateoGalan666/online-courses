...
function renderizarExitoCompra() {
  ...
  if (courseTitleVal) courseTitleVal.textContent = courseTitle;
  if (clientNameVal) clientNameVal.textContent = clientName || 'Estudiante Registrado';
  if (clientEmailVal) clientEmailVal.textContent = clientEmail || 'N/A';
