...
function actualizarHeroActions() {
  const heroBtn = document.querySelector('.hero__actions .btn--outline') as HTMLAnchorElement;
  if (!heroBtn) return;
  ...
  try {
    ...
    coursesGrid.innerHTML = `
      <div style="grid-column: 1 / -1; text-align: center; padding: 3rem; background-color: #fee2e2; border: 1px solid #fca5a5; border-radius: var(--radius-md); color: #b91c1c;">
        ...
        <p style="font-size: 0.9rem; margin-top: 0.5rem;">Verifica que el servidor esté activo. Detalle: ${(error as Error).message}</p>
      </div>
    `;
  }
}
 ...
 */
function crearTarjetaCurso(curso: Curso): HTMLElement {
