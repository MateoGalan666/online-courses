...
async function initDatabase(db: Database) {
  ...
  const colNames = tableInfo.map((col: any) => col.name);
  ...
  const usersSchema = await db.get<{ sql: string }>("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'usuarios'");
  ...
  if (userCount && userCount.count === 0) {
    ...
    await db.run(
      `INSERT INTO usuarios (nombre, email, password, rol)
       VALUES (?, ?, ?, ?)`,
      ['Administrador Antigravity', 'admin@antigravity.academy', hash, 'admin']
    );
    await db.run(
      `INSERT INTO usuarios (nombre, email, password, rol)
       VALUES (?, ?, ?, ?)`,
      ['Profesor Antigravity', 'profesor@antigravity.academy', hash, 'profesor']
    );
    await db.run(
      `INSERT INTO usuarios (nombre, email, password, rol)
       VALUES (?, ?, ?, ?)`,
      ['Estudiante Antigravity', 'estudiante@antigravity.academy', hash, 'estudiante']
    );
  ...
  const count = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM cursos');
  if (count && count.count === 0) {
    ...
    for (const course of initialCourses) {
      await db.run(
        `INSERT INTO cursos (titulo, descripcion, precio, imagen_url, temario, instructor)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [course.titulo, course.descripcion, course.precio, course.imagen_url, course.temario, course.instructor]
      );
    }
