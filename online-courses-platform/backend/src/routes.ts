...
function authenticateUser(req: Request, res: Response, next: NextFunction) {
  ...
  (req as any).user = decoded; // Contiene: { id, nombre, email, rol }
  return next();
}

// Middleware de rol específico
type UserRole = 'estudiante' | 'profesor' | 'admin';

function requireRole(rol: UserRole) {
...
router.get('/api/auth/me', authenticateUser, async (req: Request, res: Response) => {
  return res.json((req as any).user);
});

// Gestionar usuarios y credenciales (solo Admin)
router.get('/api/admin/users', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const db = await getDatabase();
...
router.post('/api/admin/users', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
...
router.put('/api/admin/users/:id', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
  const { id } = req.params;
  const { nombre, email, password, rol } = req.body;
  ...
  try {
    ...
    if (password) {
      await db.run('UPDATE usuarios SET nombre = ?, email = ?, password = ?, rol = ? WHERE id = ?', [nombre, email, hashPassword(password), rol, id]);
    } else {
...
router.delete('/api/admin/users/:id', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.get('/api/courses/my-learning', authenticateUser, requireRole('estudiante'), async (req: Request, res: Response) => {
  const user = (req as any).user;
  try {
    const db = await getDatabase();
...
router.post('/api/courses', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
  ...
  try {
    const db = await getDatabase();
    const result = await db.run(
      `INSERT INTO cursos (titulo, descripcion, precio, imagen_url, temario, instructor, codigo_html, instructor_avatar, instructor_bio, precio_original)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [titulo, finalDescripcion, parseFloat(precio), finalImagenUrl, finalTemario, finalInstructor, (codigo_html || ''), finalInstructorAvatar, finalInstructorBio, finalPrecioOriginal]
    );
...
router.put('/api/courses/:id', authenticateUser, requireAnyRole(['admin', 'profesor']), async (req: Request, res: Response) => {
  const { id } = req.params;
  const { titulo, descripcion, precio, imagen_url, temario, instructor, codigo_html, instructor_avatar, instructor_bio, precio_original } = req.body;
  ...
  try {
    ...
    await db.run(
      `UPDATE cursos 
       ...
       WHERE id = ?`,
      [titulo, finalDescripcion, parseFloat(precio), finalImagenUrl, finalTemario, finalInstructor, (codigo_html || ''), finalInstructorAvatar, finalInstructorBio, finalPrecioOriginal, id]
    );
...
router.delete('/api/courses/:id', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const db = await getDatabase();
    const courseExists = await db.get('SELECT id FROM cursos WHERE id = ?', [id]);
...
router.get('/api/profesor/courses', authenticateUser, requireRole('profesor'), async (req: Request, res: Response) => {
  const user = (req as any).user;
  try {
    const db = await getDatabase();
    const courses = await db.all(`
...
router.post('/api/admin/profesor-courses', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
...
router.post('/api/courses/:id/enroll-profesor', authenticateUser, requireRole('profesor'), async (req: Request, res: Response) => {
  const { id } = req.params;
  const user = (req as any).user;
  try {
    const db = await getDatabase();
...
router.post('/api/courses/:id/verify-key', authenticateUser, async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.post('/api/checkout', authenticateUser, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await db.run(
      `INSERT INTO pedidos (curso_id, nombre_cliente, email_cliente, estado)
       VALUES (?, ?, ?, 'completado')`,
      [curso_id, user.nombre, user.email]
    );
...
router.get('/api/courses/:id/enrollment-status', authenticateUser, async (req: Request, res: Response) => {
  const { id } = req.params;
  const user = (req as any).user;
  try {
    const db = await getDatabase();
...
router.get('/api/courses/:id/students', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const db = await getDatabase();
...
router.get('/api/admin/orders', authenticateUser, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const db = await getDatabase();
...
router.get('/api/courses/:id/lessons', authenticateUser, async (req: Request, res: Response) => {
  const { id } = req.params;
  const user = (req as any).user;
...
router.post('/api/courses/:id/lessons', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    const db = await getDatabase();
    const result = await db.run(
      `INSERT INTO lecciones (curso_id, titulo, descripcion, video_url, orden, dia, archivo_nombre, archivo_tipo, archivo_data, contenido_html)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, titulo, descripcion || '', video_url || '', orden || 0, dia || 1, archivo_nombre || '', archivo_tipo || '', archivo_data || '', contenido_html || '']
    );
...
router.put('/api/lessons/:id', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  const { titulo, descripcion, video_url, orden, dia, archivo_nombre, archivo_tipo, archivo_data, contenido_html } = req.body;
  ...
  try {
    ...
    await db.run(
      `UPDATE lecciones
       ...
       WHERE id = ?`,
      [titulo, descripcion || '', video_url || '', orden || 0, dia || 1, archivo_nombre || '', archivo_tipo || '', archivo_data || '', contenido_html || '', id]
    );
...
router.delete('/api/lessons/:id', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const db = await getDatabase();
...
router.get('/api/courses/:id/assignments', authenticateUser, async (req: Request, res: Response) => {
  const { id } = req.params;
  const user = (req as any).user;
...
router.post('/api/courses/:id/assignments', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    const db = await getDatabase();
    const result = await db.run(
      `INSERT INTO deberes (curso_id, titulo, descripcion, fecha_limite)
       VALUES (?, ?, ?, ?)`,
...
router.put('/api/assignments/:id', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  const { titulo, descripcion, fecha_limite } = req.body;
  ...
  try {
    const db = await getDatabase();
    await db.run(
      `UPDATE deberes SET titulo = ?, descripcion = ?, fecha_limite = ? WHERE id = ?`,
...
router.delete('/api/assignments/:id', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const db = await getDatabase();
...
router.post('/api/assignments/:id/submit', authenticateUser, requireRole('estudiante'), async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    ...
    await db.run(
      `INSERT INTO entregas_deberes (deber_id, estudiante_id, contenido_respuesta, archivo_nombre, archivo_tipo, archivo_data)
       VALUES (?, ?, ?, ?, ?, ?)
      ...
      [id, user.id, contenido_respuesta, archivo_nombre || '', archivo_tipo || '', archivo_data || '']
    );
...
router.get('/api/assignments/:id/submissions', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const db = await getDatabase();
...
router.post('/api/submissions/:id/grade', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.get('/curso/:id', (req: Request, res: Response) => {
  res.sendFile(path.join(frontendPath, 'curso.html'));
});
...
router.get('/estudiante', (req: Request, res: Response) => {
  res.sendFile(path.join(frontendPath, 'estudiante.html'));
});
...
router.get('/api/lessons/:id/questions', authenticateUser, async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const db = await getDatabase();
...
router.post('/api/lessons/:id/questions', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    const db = await getDatabase();
    const result = await db.run(
      `INSERT INTO preguntas_leccion (leccion_id, pregunta, opciones, respuesta_correcta) VALUES (?, ?, ?, ?)`,
      [id, pregunta, JSON.stringify(opciones), respuesta_correcta]
    );
...
router.delete('/api/questions/:id', authenticateUser, requireAnyRole(['profesor', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const db = await getDatabase();
